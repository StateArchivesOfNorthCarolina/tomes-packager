...
Todo:
    ...
    * Move private methods to the top.
"""
...
class DirectoryObject(object):
    ...
    def __init__(self, path, parent_object=None, root_object=None, depth=0):
        ...
        Args:
            - path (str):
            - parent_object (DirectoryObject): The parent folder to which the @path file belongs.
            - root_object (DirectoryObject): The root or "master" DirectoryObject under which the 
        ...
        self.dirs, self.files = self._get_assets()
        self.rdirs = self._get_recursives(self, self.list_object())
        self.rfiles = self._get_recursives(self, self.list_object(), attr="files")
    ...
    def _get_recursives(self, dir_obj, list_obj, master_obj=None, attr="dirs" ):
        """ ??? """
        ...
        if hasattr(dir_obj, attr):
            ...
            for d in dir_obj.dirs:
                self._get_recursives(d, list_obj, master_obj, attr)
